...
// Fetch assignment details from the 'ass_sub_adv' table
$sql = "SELECT ass_name, EmailAddress, ass_doc, ass_date FROM ass_sub_adv"; // Modify the SQL query to include assignment names and student emails
               ...
               if ($result->num_rows > 0) {
                   while ($row = $result->fetch_assoc()) {
                       ...
                       echo '<p>To view: <a href="' . $row['ass_doc'] . '">' . $originalFileName . '</a></p>';
                        ...
                        $scoreSubmitted = isset($row['score']) ? $row['score'] : '';
                        
                        echo '<label for="score">Score:</label>';
                        if ($scoreSubmitted !== '') {
                            // Display the score as read-only
                            echo '<input type="number" name="score" id="score" min="0" max="10" value="' . $scoreSubmitted . '" readonly>';
                        } else {
                            // Display an editable score input
...
<hr><div class="footer-copyright uk-text-center">
    Developed by | Rathinam Technical Campus
</div>
</div>
</footer>
</body>
